    ...
    if (fp) {
        ...
        while (!feof(fp)) {
            ...
            for (auto *p_src = buff; *p_src; ++p_src) {
                switch (*p_src) {
                    // filter ' ' and '\n'
                    case '0': row_valid |= true; ++v_out;                            break; 
                    case '1': row_valid |= true; adj_list[v_in].push_back(v_out++); break;
                }
            }
...
void Resources::parsePeopleTxt(std::vector<Person>& people) {
    ...
    if (fp) {
        ...
        while (!feof(fp)) {
            ...
            Person p(pid, name, year);  //it seems more efficient
            // p.setPid(pid).setName(std::string(buff)).setYear(year);
            people.push_back(p);
        }
